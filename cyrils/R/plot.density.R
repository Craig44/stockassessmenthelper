#' plot a density useful for time series of posteiors
#' @author unknown
#' @param x variable you want to plot
#' @export

plot.density = function(x,xlab="",ylab="",xlim=NULL,ylim=NULL,title="",lty=1,lwd=2,col="black",vertLines=NULL,vertLines.col=NULL,vertLines.lty=NULL,plot.labels=T,add=F){
  library(KernSmooth)
  h <- dpik(x)
  ...
  if(is.null(ylim))
    ...
    ylim.val <- ylim
    par(ann=F,xaxt="n",yaxt="n")
  if(!add) {
    plot(est,type="n",xlab=xlab,ylab=ylab,ylim= ylim.val,xlim=xlim.val)
    x.labels <- pretty(x,n=3)
    for (i in 1:length(x.labels))
    {
      if(plot.labels) text(x.labels[i],-0.6*lb.ht,as.character(x.labels[i]),cex=1)
                        if(plot.labels)  lines(rep(x.labels[i],2),c(0,-0.1*lb.ht))
    } 
    mtext(title,line=0.1,adj=0.5)
  }
  lines(est,type="l",lwd=lwd,col=col,lty=lty)
  lines(c(min(est$x),max(est$x)),rep(0,2))
        if(!is.null(vertLines)){
            for(i in 1:length(vertLines)){
                x.vertLine=vertLines[i]
                y.vertLine=approx(est$x,est$y,xout=x.vertLine)$y
                lines(x=c(x.vertLine,x.vertLine),y=c(0,y.vertLine),col=vertLines.col[i],lty=vertLines.lty[i])
            }
        }
  return (lb.ht)  
}

#' another density plotting function
#' @export
dplot<-function (..., name = T, quantiles = c(0.5), plot.mean = F, main = "",
    xlab = "", ylab = "", ylim, srtx = 0, bw = "nrd0", adjust = 1/3,
    adj = 0, fill = F, gap = 0.2,las=0)
{

  #taken from the casal files
    ...
    if (is.logical(name)) {
        if (name)
            axis(1,centers, sapply(substitute(list(...)), deparse)[2:(n +
                1)], srt = srtx, las=las,adj = if (srtx == 0)
                adj
            else 1)
    }
    else axis(1, centers, name, las=las,srt = srtx, adj = if (srtx ==
        0)
        adj
    else 1)
    invisible(centers)
