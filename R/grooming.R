...
#'
record_grooming_rule = function(df, index, catch.col, record = NULL, rule, year.col = NULL, attribute = "catch") {
  use_year = F # record values by year
  first_record = F # first record
  years = NULL;
  attributes_allowed = c("catch", "events", "relative_catch", "relative_events")
  ...
  if(is.null(record)) {
    ...
    if(use_year) {
      ...
      if(all(colnames(record)) == c("rule", attributes_allowed))
        stop(paste0("If you supply record, it needs 4 columns one for: ", paste(c("rule", attributes_allowed), collapse = ", ")))
    }
  }
  if(use_year) {
    print("year")
    ...
    new_record = NULL
    if(first_record) {
      new_record = data.frame(rule = rule, catch_removed = catch_removed, catch_percent = catch_percent, events_removed = events_removed, event_percent = event_percent)
    } else {
      ## get a temp value from the first entry
      new_record = record[1,]
      new_record$rule = rule
      new_record$catch_removed = catch_removed
      new_record$catch_percent = catch_percent
      new_record$events_removed = events_removed
      new_record$event_percent = event_percent
    }
    record = rbind(record, new_record)
    return(record)
  }
...
#'
apply_grooming_rule = function(df, index) {
  ## business rules
  if(length(index) != nrow(df))
    stop(paste0("length of index must equal rows in dataframe"))
