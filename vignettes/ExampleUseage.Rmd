...
```{r est_Growth, fig.width=5, fig.height=4, eval = F}
...
## GGplot and DPlyr tips
Some useful functions to standardise numbers to proportions by levels of another factor i.e. year.
```{r dplyr, eval=T}
df = data.frame(year = rep(c(2000, 2001), each = 30), X_val = rnorm(60, 34, 5))
## select columns that start with 'X'
sub_df = df %>% dplyr::select(starts_with("X"))
## standardise
temp_df = df %>%
  group_by(year) %>%
  mutate(stand_x_val = across(X_val, .fn = function(x){x/sum(x)}))
## alternative
temp_df_alt = df %>%
  group_by(year) %>%
  mutate(stand_x_val = across(X_val, .fn = list(~ ./sum(.))))
## check totals sum = 1
temp_df %>% group_by(year) %>% summarise(totals = sum(stand_x_val))
temp_df_alt %>% group_by(year) %>% summarise(totals = sum(stand_x_val))
```

A useful function for plotting SSB quantiles.
```{r quants, eval=T, fig.height = 5, fig.width = 5, fig.align = "center"}
## Set quantile range
p <- c(0.025, 0.5, 0.975)
labs = c("low", "mid", "upp") ## set labels
p_funs <- map(p, ~partial(quantile, probs = .x, na.rm = TRUE)) %>%
  set_names(nm = labs)
p_funs
## get data set
ssb_df = data.frame(year = rep(c(2000, 2001, 2002), each = 300), 
                    SSB = c(rnorm(300, 2000, 200), rnorm(300, 2500, 200), rnorm(300, 1500, 200)))
## run function over data set
quantile_ssb_df = ssb_df %>% 
  group_by(year) %>% 
  summarize_at(vars(SSB), funs(!!!p_funs))
## create ggplot
quant_ssb_plot = ggplot(quantile_ssb_df, aes(x = year, linetype = "dashed")) + 
  geom_ribbon(aes(ymax  = low, ymin = upp, alpha = 0.5), lwd=0) +
  theme_bw() +
  geom_line(aes(y = mid), size =2, col = "grey60") +
  xlab("Years") +
  ylab("SSB") +
  ylim(0, 3000) +
  guides(alpha = 'none', linetype = "none") +
  ggtitle("95% Quantiles") +
  scale_x_continuous(breaks=c(2000, 2001, 2002))
quant_ssb_plot
```
### Characterisation and Grooming code
There are functions which help report the effect of grooming rules `record_grooming_rule` and `apply_grooming_rule` for consistent code. It is important to see how grooming rules effect catch, effort and spatial distribution.


One diagnostic that I want to implement in future characterizations is to look at trends in nominal catch rates wihtin indiviudal discrete cells that have been fished for consecutive years. I think this will give an indication of serial depletion. We will want to look at trends in median catch rates, sample sizes, and variance in catch rates over time. Another exploratory analysis is number of cells fished in in a given year, this will give an indication of expansion or contraction in fishing which may indicate something. Below is some rough pseudo code.
...
n_tows <- rasterize(df, raster_grid, field = df$catchrate, length)
...
for(t in 1:length(years)) {
  ...
  ## ..etc
  this_year_median_catchrate <- rasterize(this_year, raster_grid, field = this_year$catchrate, median)
}
## then visualize.
```
