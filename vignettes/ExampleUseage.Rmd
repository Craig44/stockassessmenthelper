...
```{r plot_Growth, fig.width=6, fig.height=5}
...
# cvs are by length
all_cvs = cv_by_length(cv1, cv2, length_at_age)
...
```{r generate_growth_data, fig.width=6, fig.height=5}
n_samples_per_age = 5
age_length_data = NULL
for(a in 1:length(ages)) {
  lengths = r_lnorm(n_samples_per_age, length_at_age[a], cv = all_cvs[a])
...
```{r est_Growth, fig.width=6, fig.height=5}
...
```{r alt_growth_plot, fig.width=6, fig.height=5, eval=FALSE}
## concentrate on 3 ages for clarity
std_devs = log_sigma(all_cvs)
draws = 2000
ages_ndx = c(11:13)
